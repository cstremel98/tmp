...
Test(String, String__reserve_0, .disabled=false) {
  char* test_c_string = "Hello";
  String test_str = String__init(test_c_string);

  size_t original_capacity = test_str.capacity;
  size_t original_length = test_str.length;

  // Test case 1: Reserve capacity smaller than current - should do nothing
  String__reserve(&test_str, original_length - 1);
  cr_assert(test_str.capacity == original_capacity);
  cr_assert(test_str.length == original_length);
  ...
  // Verify the reserved space is zero-filled
  for (size_t i = test_str.length; i < test_str.capacity; i++) {
    cr_assert(test_str.data[i] == '\0');
  }
}
